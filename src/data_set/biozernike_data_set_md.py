...
class BiozernikeDatasetMD(Dataset):
        ...
        for i in range(len(self.descriptor_classes)):
            for j in range(i + 1, len(self.descriptor_classes)):
                self.descriptor_pairs.append([
                    [1., 0.] if self.descriptor_classes[i] == self.descriptor_classes[j] else [0., 1.],
                    i,
                    j
                ])
    ...
    def weights(self):
        p = 0.5 / sum([dp[0][0] for dp in self.descriptor_pairs])
        n = 0.5 / sum([1 - dp[0][0] for dp in self.descriptor_pairs])
